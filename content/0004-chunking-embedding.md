...
解析出来的是"能读的文本",但直接拿去切分前还有一道常被省掉的工序:**数据清洗 / 预处理**。生产里解析出来的原始文本通常带着噪声,不清洗就切分,噪声会跟着进向量、拖低召回。极客 RAG 实战课把清洗总结成三类:

| 清洗项 | 清理什么 | 为什么值得做 |
|---|---|---|
| 去冗余模板 | 合同 / 报告里重复出现的固定条款、样板段 | 重复内容占存储、重复召回到多个一模一样块 |
| 去页眉页脚 / 版权 | PDF / 网页解析带回的页眉、页脚、版权声明 | 页面级噪声与正文无关,拉低语义密度 |
| 统一格式 | 多余空行、缩进、格式不一致 | 让分块边界更稳定,避免块被空行拦腰切断 |

**和 WeKnora 的关系**:WeKnora 并没有把"数据清洗"做成一个独立显式阶段——它的质量保障主要押在 docparser 的多引擎解析选型和 chunker 的校验回退(下一节 Auto 分块里的 `ValidateChunks` 会把不合格块打回重切)。但理解这一环对你仍必要:它解释了为什么很多生产 RAG 会在"解析 → 切分"之间额外加一道净化,也让你在接入自己的文档源时知道该把脏数据堵在哪一步。
...
后面 0005 课会讲"怎么算距离"(L1/L2/余弦),这一课只需要确立:**嵌入是"人类语言 → 机器向量"的翻译层,向量空间里距离 ≈ 语义差异**。
