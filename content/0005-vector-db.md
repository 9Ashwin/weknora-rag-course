...
业内实践先后试过 Faiss、Milvus、PostgreSQL,在多次踩坑后**最终锁定装了 pgvector 插件的 PostgreSQL**,并在生产稳定运行了 12 个月。原因落在工程现实:
...
在 RAG 里最常用的是**余弦距离**:它对向量做了归一化,更贴合"语义方向相不相似",而不是"长度相不相似"。(WeKnora 主线就用余弦,见 0006 课的 `<=>`。)
