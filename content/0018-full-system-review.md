...
每个编号都对应该课文件名(如 `0006-hybrid-retrieval.md`),面试时你可以"指哪儿打哪儿"。
...
- 关键设计决策:混合检索双路互补、RRF 融合、MMR 去重、引用溯源(0006/0007/0008 课)
...
2. **再看查询侧**:用户问题是否被查询重写/扩展?有没有元数据可以缩小范围?(0009 课)
3. **再看数据侧**:解析有没有丢内容?chunk-merge 有没有误删?子块有没有独立 embedding?(0003/0004 课)
4. **最后量化**:用 NDCG@10/MRR 测排序,用 Recall 测召回,改完跑评测闭环看指标是否真升(0010/0002 课)
...
2. 顺序:① 引用溯源(0007)——每个结论可追溯到出处,这是第一道防线;② 评估指标化(0010/0013)——用 faithfulness/NDCG 持续监测;③ 检索质量兜底(0006/0006)——混合检索+重排让模型"有据可依";④ 高风险场景加 CRAG/Self-RAG 评分循环(0010)——检索质量不行就修正或拒绝回答。
